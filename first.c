...
void printResulthelp(int index, char word[100], FILE *outputfile, struct trienode* ptr){
    ...
    int k;
    for(k = 0; k< 26; k++){
        printResulthelp((index+1), word, outputfile, ptr->children[k]);
    }
...
void printResult(FILE *output){
    ...
    printResulthelp(-1, wordarr, output, root);
}
